...
void Preprocess::unitree_l1_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  bool has_time = false;
  bool has_ring = false;
  for (const auto & field : msg->fields)
  {
    ...
    has_ring = has_ring || field.name == "ring";
  }

  if (!has_time || !has_ring)
  {
    static bool warned = false;
    if (!warned)
    {
      std::cerr << "Unitree L1 PointCloud2 requires time and ring fields; "
                   "falling back to XYZ/I without motion undistortion" << std::endl;
      warned = true;
    }
    default_handler(msg);
    return;
  ...
  // Unitree L1 publishes the same x/y/z/intensity/time(float32)/ring(uint16)
  // field contract used by this handler. timestamp_unit converts time to the
  // millisecond curvature convention consumed by FAST-LIO deskewing.
  velodyne_handler(msg);
}
