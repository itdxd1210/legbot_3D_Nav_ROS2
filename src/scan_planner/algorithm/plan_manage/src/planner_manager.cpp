...
namespace scan_planner
{
  ...
  void SCANPlannerManager::initPlanModules(legbot::NodeHandle &nh, PlanningVisualization::Ptr vis)
  {
    ...
    nh.param("fsm/global_reference_mode", global_reference_mode_, std::string("piecewise_linear"));
                                                  ...
                                                  const std::vector<Eigen::Vector3d> &waypoints, const Eigen::Vector3d &end_vel, const Eigen::Vector3d &end_acc)
  {
    ...
    {
      ROS_ERROR("[SCAN] Invalid global-trajectory input.");
      return false;
    }
    ...
    for (const Eigen::Vector3d &waypoint : waypoints)
    {
      ...
      {
        ROS_ERROR("[SCAN] Global trajectory contains a non-finite waypoint.");
        return false;
      }
      if ((waypoint - points.back()).norm() > 1e-3)
    ...
    PolynomialTraj gl_traj;
    const bool use_piecewise = global_reference_mode_ != "min_snap";

    if (use_piecewise)
    {
      // Piecewise-linear reference: preserve PCT corners and stair turns.
      // The local SCAN B-spline still smooths the actual published trajectory.
      for (size_t i = 0; i + 1 < points.size(); ++i)
      {
        ...
        const double seg_time = std::max(dist / std::max(pp_.max_vel_, 1e-3), 1e-3);
        std::vector<double> cx(2), cy(2), cz(2);
        cx[0] = (p1.x() - p0.x()) / seg_time;
        cx[1] = p0.x();
        cy[0] = (p1.y() - p0.y()) / seg_time;
        cy[1] = p0.y();
        cz[0] = (p1.z() - p0.z()) / seg_time;
        cz[1] = p0.z();
        gl_traj.addSegment(cx, cy, cz, seg_time);
      }
    ...
    else
    {
      ...
      if (time.rows() == 1)
      {
        // A point-to-point route is both the first and last segment.  Scaling
        // the same duration twice makes long RViz goals unnecessarily slow
        // and badly conditions the fifth-order boundary solve.
        time(0) *= 2.0;
      }
      else
      {
                                         ...
                                         const Eigen::Vector3d &end_pos, const Eigen::Vector3d &end_vel, const Eigen::Vector3d &end_acc)
  {
    ...
    {
      ROS_ERROR("[SCAN] Invalid point-to-point global-trajectory input.");
      return false;
    }
  ...
  void SCANPlannerManager::updateTrajInfo(const UniformBspline &position_traj, const legbot::Time time_now)
  {
