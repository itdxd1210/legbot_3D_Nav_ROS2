...
namespace scan_planner
{
  ...
  void BsplineOptimizer::setParam(legbot::NodeHandle &nh)
  {
  ...
  std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    ...
    // Check every optimizable segment. For a ground robot the complete local
    // trajectory is committed motion, so an occupied final third must not be
    // deferred to a later replan cycle.
                                             ...
                                             Eigen::MatrixXd &gradient)
  {
    ...
    for (int i = 0; i < q.cols() - 1; i++)
    {
      ...
      for (int j = 0; j < 3; j++)
      {
        if (vi(j) > max_vel_)
        {
          // cout << "velocity limit exceeded" << endl;
          // cout << vi(j) << endl;
  ...
  bool BsplineOptimizer::rebound_optimize()
  {
    ...
    legbot::Time t0 = legbot::Time::now(), t1, t2;
    ...
    do
    {
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        // Validate the complete optimized trajectory, including its final
        // third, before publishing it to the GO2 command adapter.
        for (double t = tm;; t += t_step)
        {
          const double sample_t = std::min(t, tmp);
          Eigen::Vector3d pos = traj.evaluateDeBoorT(sample_t);
          Eigen::Vector3d pos_next = traj.evaluateDeBoorT(std::min(sample_t + t_step, tmp));
          flag_occ = grid_map_->getInflateOccupancy(pos, estimateSegmentYaw(pos, pos_next));
          if (flag_occ)
          {
            //cout << "hit_obs, t=" << t << " P=" << traj.evaluateDeBoorT(t).transpose() << endl;

            if (sample_t <= bspline_interval_) // First 3 control points in obstacles!
            {
              cout << cps_.points.col(1).transpose() << "\n"
                   << cps_.points.col(2).transpose() << "\n"
                   << cps_.points.col(3).transpose() << "\n"
                   << cps_.points.col(4).transpose() << endl;
              ROS_WARN("First 3 control points in obstacles! return false, t=%f", sample_t);
              return false;
            }
