...
PolynomialTraj PolynomialTraj::one_segment_traj_gen(const Eigen::Vector3d &start_pt, const Eigen::Vector3d &start_vel, const Eigen::Vector3d &start_acc,
                                                    ...
                                                    double t)
{
  // Solve the quintic boundary conditions analytically.  The former generic
  // 6x6 solve contained t^5 terms and became ill-conditioned for the 40--80 s
  // durations produced by conservative quadruped speed limits, occasionally
  // bending a forward route behind its start point.
  t = std::max(t, 1e-3);
  const double t2 = t * t;
  const double t3 = t2 * t;
  const double t4 = t3 * t;
  const double t5 = t4 * t;
  const Eigen::Vector3d delta = end_pt - start_pt;
  const Eigen::Vector3d c0 = start_pt;
  const Eigen::Vector3d c1 = start_vel;
  const Eigen::Vector3d c2 = 0.5 * start_acc;
  const Eigen::Vector3d c3 =
      (20.0 * delta - (12.0 * start_vel + 8.0 * end_vel) * t -
       (3.0 * start_acc - end_acc) * t2) /
      (2.0 * t3);
  const Eigen::Vector3d c4 =
      (-30.0 * delta + (16.0 * start_vel + 14.0 * end_vel) * t +
       (3.0 * start_acc - 2.0 * end_acc) * t2) /
      (2.0 * t4);
  const Eigen::Vector3d c5 =
      (12.0 * delta - 6.0 * (start_vel + end_vel) * t -
  ...
  for (int i = 0; i < 6; i++)
  {
    const Eigen::Vector3d coefficients[6] = {c5, c4, c3, c2, c1, c0};
    cx[i] = coefficients[i].x();
    cy[i] = coefficients[i].y();
    cz[i] = coefficients[i].z();
  }
