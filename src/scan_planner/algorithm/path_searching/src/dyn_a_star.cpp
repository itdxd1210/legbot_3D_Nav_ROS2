...
bool AStar::ConvertToIndexAndAdjustStartEndPoints(Vector3d start_pt, Vector3d end_pt, Vector3i &start_idx, Vector3i &end_idx)
{
    ...
    if (occ)
    {
        const int max_adjust_steps = std::max(1, static_cast<int>(std::ceil(0.75 / step_size_)));
        int adjust_steps = 0;
        do
        {
            if (++adjust_steps > max_adjust_steps)
            {
                ROS_WARN("[Astar] Occupied endpoint has no free cell within 0.75 m toward the start.");
                return false;
            }
            // An occupied endpoint must be brought back toward the start.
            // Advancing beyond the endpoint pushes it deeper into obstacles
            // and eventually outside the local A* grid.
...
{
    legbot::Time time_1 = legbot::Time::now();
    ++rounds_;
    ...
    while (!openSet_.empty())
    {
        ...
        {
            // legbot::Time time_2 = legbot::Time::now();
