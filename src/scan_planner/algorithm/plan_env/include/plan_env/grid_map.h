...
struct MappingParameters {
  ...
  double vis_height_, vis_height_down_, ground_height_;
  bool show_occ_time_;
  ...
  double lidar_horizontal_fov_deg_;
  bool lidar_clear_outside_fov_;
...
struct MappingData {
  ...
  bool has_ray_pose_, has_sliding_map_frame_pose_, has_cloud_;
...
public:
  ...
  void initMap(legbot::NodeHandle& nh);
...
private:
  ...
  void lidarCloudPoseCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr& cloud,
                              const nav_msgs::msg::Odometry::ConstSharedPtr& pose);
  void slidingMapFrameCallback(const nav_msgs::msg::Odometry::ConstSharedPtr& pose);
  bool isWithinPublishedHeightBand(const Eigen::Vector3d& pt_world) const;
  bool isWithinLidarHorizontalFov(const Eigen::Vector3d& pt_world) const;
  void clearOccupancyOutsideLidarFov();
  void cloudCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr& img);

  // update occupancy by raycasting
  void updateOccupancyCallback(const legbot::TimerEvent& /*event*/);
  void visCallback(const legbot::TimerEvent& /*event*/);
  ...
  legbot::Subscriber lidar_pose_sub_, sliding_map_frame_sub_, cloud_sub_;
  legbot::Publisher map_pub_, map_inf_pub_, sliding_map_bbox_pub_;
  legbot::Publisher unknown_pub_;
  legbot::Publisher depth_cloud_pub_, extrinsic_pose_pub_;
  legbot::Timer occ_timer_, vis_timer_;
...
inline int GridMap::getInflateOccupancy(Eigen::Vector3d pos, double yaw) {
  if (!mp_.collision_check_enabled_)
    return 0;
