...
GridMap::~GridMap()
{
  // Tear ROS callbacks down before message filters and the TF broadcaster.
  // Relying on global ROS destruction order caused boost::lock_error during
  // roslaunch Ctrl-C after long-running simulations.
  occ_timer_.stop();
  vis_timer_.stop();
  lidar_pose_sub_.shutdown();
  sliding_map_frame_sub_.shutdown();
  cloud_sub_.shutdown();
  if (depth_sub_)
    depth_sub_->unsubscribe();
  if (depth_pose_sub_)
    depth_pose_sub_->unsubscribe();
  if (lidar_cloud_sub_)
    lidar_cloud_sub_->unsubscribe();
  if (lidar_pose_filter_sub_)
    lidar_pose_filter_sub_->unsubscribe();
  sync_image_pose_.reset();
  sync_lidar_pose_.reset();
  depth_sub_.reset();
  depth_pose_sub_.reset();
  lidar_cloud_sub_.reset();
  lidar_pose_filter_sub_.reset();
  map_pub_.shutdown();
  map_inf_pub_.shutdown();
  sliding_map_bbox_pub_.shutdown();
  unknown_pub_.shutdown();
  depth_cloud_pub_.shutdown();
  extrinsic_pose_pub_.shutdown();
  tf_broadcaster_.reset();
}

void GridMap::initMap(legbot::NodeHandle &nh)
{
  node_ = nh;
  ...
  node_.param("grid_map/collision_check_enabled", mp_.collision_check_enabled_, true);
  ...
  node_.param("grid_map/vis_height_down", mp_.vis_height_down_, 0.15);
  ...
  node_.param("grid_map/lidar_horizontal_fov_deg", mp_.lidar_horizontal_fov_deg_, 360.0);
  node_.param("grid_map/lidar_clear_outside_fov", mp_.lidar_clear_outside_fov_, false);
  mp_.lidar_horizontal_fov_deg_ =
      std::clamp(mp_.lidar_horizontal_fov_deg_, 0.0, 360.0);
  ...
  {
    ROS_INFO_STREAM("[GridMap] LiDAR planning FOV=" << mp_.lidar_horizontal_fov_deg_
                    << " deg, clear outside FOV="
                    << (mp_.lidar_clear_outside_fov_ ? "true" : "false"));
  }
  ...
  /* init callback */
  tf_broadcaster_.reset(new legbot_tf::TransformBroadcaster);
  if (mp_.sensor_type_ == "depth")
  {
    ...
    depth_pose_sub_.reset(
        new message_filters::Subscriber<nav_msgs::msg::Odometry>(node_.get(), "/grid_map/sensor_pose", rmw_qos_profile_sensor_data));
  ...
  {
    // FAST-LIO publishes registered cloud and odometry at the same scan-end
    // timestamp. Never raycast a cloud from the latest unrelated pose: walking
    // motion otherwise displaces floor and near-obstacle occupancy.
    lidar_cloud_sub_.reset(new message_filters::Subscriber<sensor_msgs::msg::PointCloud2>(
        node_.get(), "/grid_map/cloud", rmw_qos_profile_sensor_data));
    lidar_pose_filter_sub_.reset(new message_filters::Subscriber<nav_msgs::msg::Odometry>(
        node_.get(), "/grid_map/sensor_pose", rmw_qos_profile_sensor_data));
    sync_lidar_pose_.reset(new message_filters::Synchronizer<SyncPolicyLidarPose>(
        SyncPolicyLidarPose(100), *lidar_cloud_sub_, *lidar_pose_filter_sub_));
    sync_lidar_pose_->registerCallback(std::bind(&GridMap::lidarCloudPoseCallback, this,
                                                std::placeholders::_1, std::placeholders::_2));
  }

  sliding_map_frame_sub_ =
      node_.subscribe<nav_msgs::msg::Odometry>("/grid_map/body_pose", 50, &GridMap::slidingMapFrameCallback, this);

  occ_timer_ = node_.createTimer(legbot::Duration(0.05), &GridMap::updateOccupancyCallback, this);
  vis_timer_ = node_.createTimer(legbot::Duration(0.05), &GridMap::visCallback, this);
  ...
  sliding_map_bbox_pub_ = node_.advertise<visualization_msgs::msg::Marker>("/grid_map/sliding_map_bbox", 10);

  unknown_pub_ = node_.advertise<sensor_msgs::msg::PointCloud2>("/grid_map/unknown", 10);
  depth_cloud_pub_ = node_.advertise<sensor_msgs::msg::PointCloud2>("/grid_map/depth_cloud", 10);
  extrinsic_pose_pub_ = node_.advertise<nav_msgs::msg::Odometry>("/grid_map/sensor_pose_extrinsic", 10);
...
void GridMap::visCallback(const legbot::TimerEvent & /*event*/)
{
...
void GridMap::updateOccupancyCallback(const legbot::TimerEvent & /*event*/)
{
  if (!md_.occ_need_update_)
    return;

  /* update occupancy */
  // legbot::Time t1, t2, t3, t4;
  // t1 = legbot::Time::now();
                                ...
                                const nav_msgs::msg::Odometry::ConstSharedPtr &pose)
{
  ...
  if (!std::isfinite(ray_q.norm()) || ray_q.norm() < 0.5 || ray_q.norm() > 1.5)
    return;
  ray_q.normalize();
...
void GridMap::sensorPoseCallback(const nav_msgs::msg::Odometry::ConstSharedPtr &pose_msg)
{
  ...
  if (!std::isfinite(ray_pos.x()) || !std::isfinite(ray_pos.y()) || !std::isfinite(ray_pos.z()) ||
      ray_pos.cwiseAbs().maxCoeff() > 10000.0)
    return;
...
void GridMap::lidarCloudPoseCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &cloud,
                                     const nav_msgs::msg::Odometry::ConstSharedPtr &pose)
{
...
void GridMap::slidingMapFrameCallback(const nav_msgs::msg::Odometry::ConstSharedPtr &pose)
{
  ...
  const geometry_msgs::msg::Quaternion &ori = pose->pose.pose.orientation;
  if (!std::isfinite(pos.x) || !std::isfinite(pos.y) || !std::isfinite(pos.z) ||
      !std::isfinite(ori.x) || !std::isfinite(ori.y) || !std::isfinite(ori.z) ||
      !std::isfinite(ori.w) ||
      std::max({std::abs(pos.x), std::abs(pos.y), std::abs(pos.z)}) > 10000.0)
    return;

  Eigen::Quaterniond body_q(ori.w, ori.x, ori.y, ori.z);
  if (body_q.squaredNorm() < 1e-8)
    return;

  md_.sliding_map_frame_pos_ = Eigen::Vector3d(pos.x, pos.y, pos.z);
  md_.sliding_map_frame_q_ = body_q.normalized();
...
void GridMap::cloudCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &img)
{
  ...
  // SCAN may intentionally use a narrower field of view than FAST-LIO. Clear
  // occupied cells outside the current LiDAR FOV so inflation, optimization
  // and collision checking cannot keep using historical rear observations.
  // FAST-LIO still receives the original 360-degree cloud for localization.
  if (mp_.lidar_clear_outside_fov_ && mp_.lidar_horizontal_fov_deg_ < 359.999)
    clearOccupancyOutsideLidarFov();
  ...
  for (size_t i = 0; i < latest_cloud.points.size(); ++i)
  {
    ...
    if (!isWithinLidarHorizontalFov(pt_world))
      continue;
...
bool GridMap::isWithinLidarHorizontalFov(const Eigen::Vector3d &pt_world) const
{
  if (mp_.lidar_horizontal_fov_deg_ >= 359.999)
    return true;

  const Eigen::Vector3d pt_sensor =
      md_.ray_q_.conjugate() * (pt_world - md_.ray_pos_);
  const double planar_range = std::hypot(pt_sensor.x(), pt_sensor.y());
  if (planar_range < 1e-6)
    return true;

  constexpr double kPi = 3.14159265358979323846;
  const double half_fov_rad =
      0.5 * mp_.lidar_horizontal_fov_deg_ * kPi / 180.0;
  const double min_forward_cos = std::cos(half_fov_rad);
  return pt_sensor.x() / planar_range >= min_forward_cos - 1e-9;
}

void GridMap::clearOccupancyOutsideLidarFov()
{
  for (int addr = 0; addr < static_cast<int>(md_.occupancy_buffer_.size()); ++addr)
  {
    ...
    indexToPos(id, pos);
    if (!isWithinLidarHorizontalFov(pos))
      resetCellByAddress(addr);
  }
}

bool GridMap::isWithinPublishedHeightBand(const Eigen::Vector3d &pt_world) const
{
  // Publish the base-height band used for rigid-body planning in RViz. The
  // internal 3-D grid remains untouched.
  double reference_z = 0.0;
  if (md_.has_sliding_map_frame_pose_)
    reference_z = md_.sliding_map_frame_pos_.z();
  else if (md_.has_ray_pose_)
    reference_z = md_.ray_pos_.z();
  else
    return true;
  return pt_world.z() >= reference_z - std::max(0.0, mp_.vis_height_down_) &&
         pt_world.z() <= reference_z + std::max(0.0, mp_.vis_height_);
}
  ...
  for (int x = min_cut(0); x <= max_cut(0); ++x)
    for (int y = min_cut(1); y <= max_cut(1); ++y)
      for (int z = min_cut(2); z <= max_cut(2); ++z)
      {
        ...
        if (!isWithinPublishedHeightBand(pos))
          continue;
        pt.x = pos(0);
        pt.y = pos(1);
        pt.z = pos(2);
...
void GridMap::publishSlidingMapFrame()
{
  ...
  const legbot::Time stamp = legbot::Time::now();
  static legbot::Time last_stamp;
  static bool have_last_stamp = false;
  if (have_last_stamp && stamp == last_stamp)
    return;
  last_stamp = stamp;
  have_last_stamp = true;

  legbot_tf::Transform transform;
  transform.setOrigin(legbot_tf::Vector3(md_.sliding_map_frame_pos_.x(), md_.sliding_map_frame_pos_.y(),
                                  md_.sliding_map_frame_pos_.z()));
  transform.setRotation(legbot_tf::Quaternion::getIdentity());
  tf_broadcaster_->sendTransform(
      legbot_tf::StampedTransform(transform, stamp, mp_.frame_id_, mp_.sliding_map_frame_id_));
}
...
void GridMap::publishDepthCloud()
{
  ...
  sensor_msgs::msg::PointCloud2 cloud_msg;
  pcl::toROSMsg(cloud, cloud_msg);
  cloud_msg.header.stamp = legbot::Time::now();
