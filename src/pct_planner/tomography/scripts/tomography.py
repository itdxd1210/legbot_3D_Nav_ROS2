...
rclpy.init()
node = rclpy.create_node('pointcloud_tomography')
...
class Tomography(object):
    def __init__(self, cfg, scene_cfg):
        self.export_dir = os.path.join(os.environ.get('LEGBOT_DATA_DIR', os.path.join(os.getcwd(), 'data')), 'tomogram') + os.sep
    ...
    def initROS(self):
        ...
        self.pointcloud_pub = node.create_publisher(PointCloud2, pointcloud_topic, QoSProfile(depth=1, durability=DurabilityPolicy.TRANSIENT_LOCAL))
        ...
        for i in range(self.n_slice):
            layer_G_pub = node.create_publisher(PointCloud2, layer_G_topic + str(i), QoSProfile(depth=1, durability=DurabilityPolicy.TRANSIENT_LOCAL))
            self.layer_G_pub_list.append(layer_G_pub)
            layer_C_pub = node.create_publisher(PointCloud2, layer_C_topic + str(i), QoSProfile(depth=1, durability=DurabilityPolicy.TRANSIENT_LOCAL))
    ...
    def loadPCD(self, pcd_file):
        ...
        loginfo("Map center: [%.2f, %.2f]", self.center[0], self.center[1])
        loginfo("Dim_x: %d", self.map_dim_x)
        loginfo("Dim_y: %d", self.map_dim_y)
        loginfo("Num slices init: %d", n_slice_init)
    ...
    def process(self, points):        
        ...
        loginfo("Num slices simp: %d", layers_g.shape[0])
        loginfo("Num repeats (for benchmarking only): %d", n_repeat)
        loginfo(" -- avg t_map  (ms): %f", t_map / n_repeat)
        loginfo(" -- avg t_trav (ms): %f", t_trav / n_repeat)
        loginfo(" -- avg t_simp (ms): %f", t_simp / n_repeat)
        loginfo(" -- avg t_all  (ms): %f", t_all / n_repeat)
    ...
    def exportTomogram(self, tomogram, map_file):        
        ...
        loginfo("Tomogram exported: %s", file_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scene', type=str, default='Go2',
