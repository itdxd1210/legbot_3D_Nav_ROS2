...
namespace ego_planner
{
  ...
  std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    ...
    {
      ROS_ERROR("[EGO] Initial trajectory has fewer than two control points.");
      return {};
    }
    const double mean_spacing =
        (init_points.col(0) - init_points.rightCols(1)).norm() / (init_points.cols() - 1);
    const double step_size = mean_spacing > 1e-6
                                 ? std::min(0.5, grid_map_->getResolution() / mean_spacing / 2.0)
                                 : 0.5;
  ...
  bool BsplineOptimizer::check_collision_and_rebound(void)
  {
    ...
    for (int i = order_ - 1; i <= i_end; ++i)
    {
      ...
      if (occ)
      {
        ...
        {
          ROS_ERROR("[EGO] Initial trajectory enters an occupied voxel before a free incoming control point "
                    "(control point %d: [%.3f %.3f %.3f]). Skip this invalid initial guess.",
                    i, cps_.points(0, i), cps_.points(1, i), cps_.points(2, i));
