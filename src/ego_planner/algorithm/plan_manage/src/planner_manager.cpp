...
namespace ego_planner
{
  ...
  void EGOPlannerManager::initPlanModules(legbot::NodeHandle &nh, PlanningVisualization::Ptr vis)
  {
                                        ...
                                        Eigen::Vector3d local_target_vel, bool flag_polyInit, bool flag_randomPolyTraj)
  {
        ...
        !local_target_pt.allFinite() || !local_target_vel.allFinite())
    {
      ...
      ROS_ERROR("[EGO] Refuse to optimize a non-finite local planning state (consecutive failures: %d).",
                continous_failures_count_);
      return false;
    }

    const int start_occupancy = grid_map_->getInflateOccupancy(start_pt);
    if (start_occupancy != 0)
    {
      ++continous_failures_count_;
      ROS_ERROR("[EGO] Local-plan start is not free (occupancy=%d, xyz=[%.3f %.3f %.3f], consecutive failures=%d). "
                "Skip L-BFGS and wait for a valid map update.",
                start_occupancy, start_pt.x(), start_pt.y(), start_pt.z(), continous_failures_count_);
      return false;
    }

    legbot::Time t_start = legbot::Time::now();
    legbot::Duration t_init, t_opt, t_refine;
    ...
    do
    {
      ...
      //重复inital
        ...
        double t;
        double t_cur = (legbot::Time::now() - local_data_.start_time_).toSec();
    ...
    t_init = legbot::Time::now() - t_start;
    ...
    if (!flag_step_1_success)
    {
      ...
      ++continous_failures_count_;
      ROS_ERROR("[EGO] Local optimization failed (consecutive failures: %d).", continous_failures_count_);
      return false;
    }
    
    visualization_->displayOptimalList( ctrl_pts, vis_id );
    //计算优化时间
    t_opt = legbot::Time::now() - t_start;
    t_start = legbot::Time::now();
    ...
    t_refine = legbot::Time::now() - t_start;

    // save planned results
    updateTrajInfo(pos, legbot::Time::now());
  ...
  bool EGOPlannerManager::EmergencyStop(Eigen::Vector3d stop_pos)
  {
    ...
    updateTrajInfo(UniformBspline(control_points, 3, 1.0), legbot::Time::now());
  ...
  {
    constexpr double kMinSegmentTime = 1e-3;
    constexpr double kDuplicateEps = 1e-3;
    if (waypoints.empty() || pp_.max_vel_ <= 1e-6 || !start_pos.allFinite() ||
        !start_vel.allFinite() || !start_acc.allFinite() || !end_vel.allFinite() || !end_acc.allFinite())
    {
      ROS_ERROR("[EGO] Invalid global-trajectory input (empty waypoints, non-finite state, or max_vel <= 0).");
      return false;
    }
    ...
    for (const Eigen::Vector3d &waypoint : waypoints)
    {
      ...
      {
        ROS_ERROR("[EGO] Global trajectory contains a non-finite waypoint.");
        return false;
      }
      if ((waypoint - points.back()).norm() > kDuplicateEps)
        points.push_back(waypoint);
    }

    if (points.size() < 2)
    {
      ROS_INFO("[EGO] Global-trajectory goal is already at the current position.");
      return false;
    }
    ...
    {
      time(i) = std::max(kMinSegmentTime,
                         (pos.col(i + 1) - pos.col(i)).norm() / pp_.max_vel_);
    }
  ...
  {
    if (pp_.max_vel_ <= 1e-6 || !start_pos.allFinite() || !start_vel.allFinite() ||
        !start_acc.allFinite() || !end_pos.allFinite() || !end_vel.allFinite() || !end_acc.allFinite())
    {
      ROS_ERROR("[EGO] Invalid point-to-point global-trajectory input.");
      return false;
    }
    if ((end_pos - start_pos).norm() <= 1e-3)
      return false;
    ...
    for (int i = 0; i < pt_num - 1; ++i)
    {
      time(i) = std::max(1e-3, (pos.col(i + 1) - pos.col(i)).norm() / pp_.max_vel_);
    }
  ...
  void EGOPlannerManager::updateTrajInfo(const UniformBspline &position_traj, const legbot::Time time_now)
  {
