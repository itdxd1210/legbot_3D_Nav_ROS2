...
namespace rl_quadruped_controller
{
    ...
    {
        const auto input = *input_buffer_.readFromRT();
        ctrl_interfaces_.control_inputs_ = input.msg;
        if (emergency_stop_latched_.load(std::memory_order_acquire)) {
            ctrl_interfaces_.control_inputs_.lx = 0;
            ctrl_interfaces_.control_inputs_.ly = 0;
            ctrl_interfaces_.control_inputs_.rx = 0;
            ctrl_interfaces_.control_inputs_.ry = 0;
            ...
            if (current_state_ && current_state_->state_name != FSMStateName::PASSIVE) {
                ...
                next_state_name_ = FSMStateName::PASSIVE;
                mode_ = FSMMode::NORMAL;
                RCLCPP_ERROR(
                    get_node()->get_logger(),
                    "GO2 EMERGENCY STOP ACTIVE: controller entered latched passive damping");
            }
            return controller_interface::return_type::OK;
        }
        if (std::chrono::steady_clock::now() - input.received > std::chrono::milliseconds(500)) {
            ctrl_interfaces_.control_inputs_.lx = 0;
            ctrl_interfaces_.control_inputs_.ly = 0;
            ctrl_interfaces_.control_inputs_.rx = 0;
            ctrl_interfaces_.control_inputs_.ry = 0;
        ...
        {
            try { current_state_->run(time, period); }
            catch (const std::exception &error) {
                RCLCPP_ERROR(get_node()->get_logger(), "GO2 control failure: %s", error.what());
                current_state_->exit();
        ...
        const rclcpp_lifecycle::State& /*previous_state*/)
    {
        robot_description_subscription_ = get_node()->create_subscription<std_msgs::msg::String>(
            "/go2/robot_description", rclcpp::QoS(rclcpp::KeepLast(1)).transient_local(),
        ...
        control_input_subscription_ = get_node()->create_subscription<control_input_msgs::msg::Inputs>(
            ...
            {
                if (!std::isfinite(msg->lx) || !std::isfinite(msg->ly) || !std::isfinite(msg->rx)) return;
                input_buffer_.writeFromNonRT(ReceivedInput{*msg, std::chrono::steady_clock::now()});
            });

        emergency_stop_subscription_ = get_node()->create_subscription<std_msgs::msg::Bool>(
            "/go2/emergency_stop", rclcpp::QoS(1).reliable().transient_local(),
            [this](const std_msgs::msg::Bool::SharedPtr msg)
            {
                if (!msg->data) return;
                const bool already_latched = emergency_stop_latched_.exchange(
                    true, std::memory_order_acq_rel);
                if (!already_latched) {
                    RCLCPP_ERROR(
                        get_node()->get_logger(),
                        "GO2 EMERGENCY STOP LATCHED; restart real.launch.py to clear it");
                }
            });
        ...
        const rclcpp_lifecycle::State& /*previous_state*/)
    {
        ...
        std::ostringstream joint_order;
        for (size_t index = 0; index < ctrl_interfaces_.joint_position_state_interface_.size(); ++index)
        {
            if (index != 0) joint_order << ", ";
            joint_order << ctrl_interfaces_.joint_position_state_interface_[index].get().get_prefix_name();
        }
        RCLCPP_INFO(get_node()->get_logger(), "RL joint vector order: [%s]", joint_order.str().c_str());

        std::ostringstream imu_order;
        for (size_t index = 0; index < ctrl_interfaces_.imu_state_interface_.size(); ++index)
        {
            if (index != 0) imu_order << ", ";
            imu_order << ctrl_interfaces_.imu_state_interface_[index].get().get_interface_name();
        }
        RCLCPP_INFO(get_node()->get_logger(), "RL IMU vector order: [%s]", imu_order.str().c_str());
