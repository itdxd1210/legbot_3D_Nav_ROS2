...
struct ModelParams
{
    ...
    std::string history_layout = "frame_major";
    double damping;
    double stiffness;
    double action_scale;
...
private:
    ...
    [[nodiscard]] std::vector<int> observationTermDimensions() const;
    ...
    */
    ...
    // RL inference modules. Exactly one backend is initialized from config.yaml.
    torch::jit::script::Module model_;
    Ort::Env onnx_env_{ORT_LOGGING_LEVEL_WARNING, "go2_policy"};
    Ort::SessionOptions onnx_session_options_;
    std::unique_ptr<Ort::Session> onnx_session_;
    std::string onnx_input_name_;
    std::string onnx_output_name_;
    bool use_rl_thread_ = false;
    unsigned inference_tick_ = 0;
